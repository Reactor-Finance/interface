...
const dialogContentVariants = cva(
  "fixed left-[50%] border border-neutral-700  z-50 grid mx-1 w-full max-w-lg translate-x-[-50%] " +
    " gap-4  bg-neutral-1000 p-5 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out " +
    " data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95" +
    " data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 " +
    " data-[state=open]:slide-in-from-left-1/2 rounded-md sm:rounded-lg",
  {
    variants: {
      position: {
...
const DialogContent = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content> &
    VariantProps<typeof dialogContentVariants> & { removeClose?: boolean }
>(({ className, children, position, removeClose, ...props }, ref) => (
  <DialogPortal>
    <DialogOverlay />
    ...
    >
      ...
      <DialogPrimitive.Close
        className={`absolute ${removeClose ? "hidden" : ""} right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100
